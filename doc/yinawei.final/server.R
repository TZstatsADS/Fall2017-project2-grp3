...
library(shiny)
data <- read.csv("clean_state_disaster_1.csv")
...
shinyServer(function(input, output, session) {
  ...
  yearData <- reactive({
    ...
    df2 <- data %>%
      ...
      select(State, age, Severe.Storm.s.,
             Region, Population) %>%
      arrange(Region)
  })
  ...
  output$view <- renderGvis({
    ...
    gvisMotionChart(data, xvar="Flood",yvar="age-storm",idvar='State',timevar = 'Year', sizevar='Population',colorvar = 'Region', options=list(width="800", height="800"))
  })
  ## end MotionChart
